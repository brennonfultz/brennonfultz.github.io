...
//Brennon Fultz CS 300 Project Two / CS 499 Enhancement Two
...
void loadCourses(BinarySearchTree& bst, unordered_map<string, Course>& courseTable, const string& filename) {
    ...
    while (getline(file, line)) { // read line from file
        ...
        bst.Insert(course);
        courseTable[courseId] = course; //add course to hashtable
    }
...
//new search function for hashtable
void searchCourseTable(unordered_map<string, Course>& courseTable, string& courseToFind) {
    if (courseTable.find(courseToFind) != courseTable.end()) { //search table by ID
        Course foundCourse = courseTable[courseToFind];
        cout << "Course ID:" << foundCourse.courseId << endl; //print details
        cout << "Title: " << foundCourse.title << endl;
        cout << "Prerequisite 1: " << foundCourse.prereq1 << endl;
        cout << "Prerequisite 2: " << foundCourse.prereq2 << endl;
    }
    else { //inform user if course not found
        cout << "Course not found\n";
    }
}
...
int main() {
    ...
    while (choice != 9) { //loop for menu
        ...
        case 1:
            cout << "Enter filename:\n"; //take filename input 
            cin >> filename;
            loadCourses(bst, courseTable, filename); //call method to load csv
            break;
        case 2:
        ...
        case 3:
            ...
            cin >> courseToFind;
            searchCourseTable(courseTable, courseToFind); //call new hashtable search function
            break;
        case 9:
